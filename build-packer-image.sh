...
if [ "${CIRRUS_PR:-}" != "" ]; then
  TAG=$CIRRUS_PR
  export IMAGE_FAMILY=
elif [ $CIRRUS_BRANCH == "lt-base-windows" ]; then
  export IMAGE_FAMILY=lt-base-windows
else
...
GOOGLE_APPLICATION_CREDENTIALS=account.json ./packer build -force windows/lt-base-windows.json
